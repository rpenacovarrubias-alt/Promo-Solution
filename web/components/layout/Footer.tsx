import Link from 'next/link'
import Image from 'next/image'

export function Footer() {
  return (
    <footer className="bg-navy-700 text-white mt-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">

          {/* Brand */}
          <div className="md:col-span-1">
            {/* El logo trae su propio fondo claro (no es transparente) —
               se envuelve en una tarjeta del mismo tono para que no choque con el navy */}
            <div className="inline-block bg-[#F7F6F1] rounded-lg px-3 py-2 mb-3">
              <Image src="/logo.png" alt="Promo Solution" width={300} height={90} className="h-8 w-auto" />
            </div>
            <p className="text-navy-200 text-sm leading-relaxed">
              Tu marca en mano de todos. Productos promocionales y uniformes de calidad para empresas.
            </p>
            <div className="flex gap-3 mt-4">
              <a href="https://www.facebook.com/profile.php?id=61585560026704"
                target="_blank" rel="noopener noreferrer"
                className="text-navy-300 hover:text-gold-400 transition-colors">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
              </a>
              <a href="https://www.instagram.com/promosolution_mx/"
                target="_blank" rel="noopener noreferrer"
                className="text-navy-300 hover:text-gold-400 transition-colors">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                </svg>
              </a>
            </div>
          </div>

          {/* Catálogo */}
          <div>
            <h3 className="text-gold-400 font-semibold text-sm mb-3">Catálogo</h3>
            <ul className="space-y-2 text-sm text-navy-200">
              {['Bebidas', 'Tecnología', 'Uniformes', 'Textiles', 'Oficina', 'Ecológicos'].map(c => (
                <li key={c}>
                  <Link href={`/catalogo?categoria=${encodeURIComponent(c)}`}
                    className="hover:text-gold-400 transition-colors">{c}</Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Empresa */}
          <div>
            <h3 className="text-gold-400 font-semibold text-sm mb-3">Empresa</h3>
            <ul className="space-y-2 text-sm text-navy-200">
              <li><Link href="/contacto" className="hover:text-gold-400 transition-colors">Contacto</Link></li>
              <li><Link href="/cotizar" className="hover:text-gold-400 transition-colors">Solicitar cotización</Link></li>
              <li><Link href="/privacidad" className="hover:text-gold-400 transition-colors">Aviso de Privacidad</Link></li>
            </ul>
          </div>

          {/* Contacto */}
          <div>
            <h3 className="text-gold-400 font-semibold text-sm mb-3">Contacto</h3>
            <ul className="space-y-2 text-sm text-navy-200">
              <li className="flex items-center gap-2">
                <svg className="w-4 h-4 text-gold-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                </svg>
                ventas@promosolution.com.mx
              </li>
              <li className="flex items-center gap-2">
                <svg className="w-4 h-4 text-gold-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
                </svg>
                WhatsApp / Telegram: Julio
              </li>
            </ul>
          </div>
        </div>

        <div className="border-t border-navy-600 mt-10 pt-6 text-center text-xs text-navy-300">
          © {new Date().getFullYear()} Promo Solutions. Todos los derechos reservados.
        </div>
      </div>
    </footer>
  )
}
