...
import { Document, Page, View, Text, Image, Svg, Rect, StyleSheet } from '@react-pdf/renderer'
...
// Bandas como View de flexbox (ancho en %) dejaban costuras blancas: el layout
// (yoga) redondea cada ancho a puntos enteros por separado y el acumulado de
// redondeos abre huecos de sub-punto entre bandas. Un solo <Svg> con <Rect>
// dibuja todo en un mismo sistema de coordenadas — sin layout independiente
// por banda — y el solape de 0.6 unidades entre rects cierra cualquier costura
// de antialiasing que quede.
// `width`/`height` explícitos (puntos reales, no %) — necesarios para el
// footer: ahí el Svg vive dentro del callback `render` de un View `fixed`,
// que react-pdf procesa en una pasada de layout aparte por página donde el
// top/left/right/bottom:0 en % nunca llegaba a resolver el ancho del padre
// (se encogía al ancho del texto). Con medidas fijas no depende de eso. El
// header sigue usando % porque ahí sí funciona (no está detrás de `render`).
const GRADIENT_BANDS = 80
function GradientBackground({ stops, width, height }) {
  const e = React.createElement
  const fixedSize = width != null && height != null
  const w = fixedSize ? width : 100
  const h = fixedSize ? height : 100
  return e(
    Svg,
    {
      style: fixedSize
      ...
      ...(fixedSize ? { width, height } : { viewBox: '0 0 100 100', preserveAspectRatio: 'none' }),
    },
    ...Array.from({ length: GRADIENT_BANDS }, (_, i) => {
      const t0 = (i / GRADIENT_BANDS) * w
      const t1 = ((i + 1) / GRADIENT_BANDS) * w
      const mid = (i + 0.5) / GRADIENT_BANDS
      return e(Rect, {
        key: i,
        x: t0 - 0.3,
        y: 0,
        width: t1 - t0 + 0.6,
        height: h,
        fill: colorAtStop(stops, mid),
      })
    }),
  )
}

const PAGE_WIDTH = 595.28 // A4, en puntos — igual que node_modules/@react-pdf/layout PAGE_SIZES.A4
...
export function QuotePdfDocument({ quote, logoUrl }) {
  ...
  return e(
    ...
    e(
      ...
      e(
        View,
        { style: { position: 'relative', width: '100%' }, fixed: true },
      ...
      e(
        ...
        e(
          ...
          e(
            ...
            quote.client.phone ? e(Text, { style: styles.value }, sanitizeText(quote.client.phone)) : null,
          ...
          e(
            ...
            e(Text, { style: [styles.label, styles.textRight] }, 'Promo Solution'),
            e(Text, { style: [styles.value, styles.textRight] }, 'Juan José Peña'),
            e(Text, { style: [styles.value, styles.textRight] }, 'WhatsApp: 553 790 575'),
            e(Text, { style: [styles.value, styles.textRight] }, 'ventas@promosolution.com.mx'),
          ),
        ),
        ...
        e(
          ...
          ...quote.items.map((item, index) => {
            ...
            // Referencia propia por línea (no el SKU real del proveedor — ese
            // no debe llegar al cliente) para que pueda mencionar "el item
            // COT-XXXXXX-02" al platicar contigo sobre la cotización.
            const itemRef = `${folio(quote.id)}-${String(index + 1).padStart(2, '0')}`
            return e(
              View,
              ...
              e(
                ...
                e(Text, { style: styles.tdSub }, itemRef),
              ),
              e(
                View,
      ...
      e(
        ...
        { style: styles.footer, fixed: true, render: ({ pageNumber, totalPages }) =>
          e(
            View,
            { style: { position: 'relative', width: PAGE_WIDTH, height: FOOTER_HEIGHT } },
            e(GradientBackground, { stops: FOOTER_STOPS, width: PAGE_WIDTH, height: FOOTER_HEIGHT }),
            e(
              View,
              { style: [styles.footerStrip, { position: 'absolute', top: 0, left: 0, width: PAGE_WIDTH }] },
              e(
                View,
                null,
