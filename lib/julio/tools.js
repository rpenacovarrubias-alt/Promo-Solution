...
// Julio (Telegram/WhatsApp) no conoce todavía al cliente cuando lista productos
// — el teléfono/email recién se piden dentro de crear_cotizacion, más adelante
// en la conversación (ver agent.js) — así que, igual que el catálogo público sin
// sesión (routes/public/products.js), cotiza con el mismo default que
// Client.markupPercent en el schema (33%), NO con Category.utilityPercent: ese
// campo ya no participa en ningún cálculo de precio y está en 0% en casi todas
// las categorías, lo que hacía que Julio cotizara casi el precio crudo del
// proveedor a cualquiera. Cuando se conecte Julio a la identidad real del
// cliente, este default pasa a ser el fallback y el markupPercent del cliente
// manda, igual que ya hace lib/quotes.js.
...
export function mapProduct(p) {
  return {
    ...
    precio: calcFinalPrice(p.basePrice, GUEST_MARKUP_PERCENT),
