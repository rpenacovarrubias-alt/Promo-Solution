...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const nullExcluded = new Set(
      Object.entries(cfg.componentSrcMap ?? {}).filter(([, v]) => v === null).map(([k]) => k)
    );
    const toPascal = (s) => s.split('-').map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join('');
    const filteredComps = comps.filter(p => {
      const raw = basename(p).replace(/\.(tsx|jsx|ts|js)$/, '');
      return !nullExcluded.has(raw) && !nullExcluded.has(toPascal(raw));
    });
    entry = join(OUT, '.pkg-entry.mjs');
    writeFileSync(
      entry,
      filteredComps.map((p) => `export * from ${JSON.stringify(p)};`).join('\n') + '\n'
    );
    synthEntry = true;
    console.error(
      `[NO_DIST] no built entry — synthesizing from ${filteredComps.length} src files (run the package's build for best results)`,
    );
  }
